  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        const response = await fetch('/api/frigate/mqtt?type=events');
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      <div className="container mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-3xl font-bold">Frigate Events</h1>
                  ...
                  {event.before && (
                    <div className="text-sm">
                      ...
                      <span className="text-gray-600">
                        {event.before.camera} - {event.before.label}
                        {event.before.score && ` (${formatScore(event.before.score)})`}
                      </span>
                    </div>
                  )}
                  {event.after && (
                    <div className="text-sm">
                      ...
                      <span className="text-gray-600">
                        ...
                        {event.after.score && ` (${formatScore(event.after.score)})`}
                      </span>
                    </div>
                  )}
                </div>
